 ...
 */
LPDIRECT3DDEVICE9 CreateD3DDevice(HWND hWnd, LPDIRECT3D9 pD3D, D3DPRESENT_PARAMETERS * pPresentationParameters)
{
...
{
	// Create the window
	return CreateWindow(strClassName, strWindowTitle, CS_HREDRAW | CS_VREDRAW | WS_OVERLAPPEDWINDOW,
 ...
 */
HRESULT LoadTerrain(LPDIRECT3DDEVICE9 pd3dDevice, LPDIRECT3DTEXTURE9 * ppGrassTexture, LPDIRECT3DVERTEXBUFFER9 * ppGrassVB)
{
 ...
 */
HRESULT CreateInputDevices(LPDIRECTINPUT8 pDI, HWND hWnd, LPDIRECTINPUTDEVICE8 * ppMouse, LPDIRECTINPUTDEVICE8 * ppKeyboard)
{
 ...
 *   @return Result code
 */
HRESULT UpdateInput(LPDIRECTINPUTDEVICE8 pKeyboard, LPDIRECTINPUTDEVICE8 pMouse, BYTE * keys, DIMOUSESTATE * pMs)
{
	// Get device information
	if (SUCCEEDED(pKeyboard->GetDeviceState(sizeof(BYTE) * 256, keys)) && SUCCEEDED(pMouse->GetDeviceState(sizeof(DIMOUSESTATE), pMs)))
		return S_OK;
...
{
	if (pPlayer->pWalkAnimation)		pPlayer->pWalkAnimation->Release();
	if (pPlayer->pIdleAnimation)		pPlayer->pIdleAnimation->Release();
	if (pPlayer->pRunAnimation)		pPlayer->pRunAnimation->Release();
	if (pPlayer->pController)		pPlayer->pController->Release();
...
HRESULT ConnectToServer(SOCKET sSocket, HANDLE hRecvEvent)
{
	...
	LogOnMessage packet;
	if (SOCKET_ERROR == sendto(sSocket, (CHAR*)&packet, sizeof(packet), 0, (LPSOCKADDR)&addr, sizeof(SOCKADDR_IN)))		return E_FAIL;

	// Wait for a reply or for a few seconds to pass
	if (WAIT_TIMEOUT == WSAWaitForMultipleEvents(1, &hRecvEvent, TRUE, 3000, FALSE)) return E_FAIL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
		...
		SUCCEEDED(player.mesh.CloneAnimationController(2, &player.pController)))
	{
		...
		{
			// Clear the scene
			pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, BACKGROUND_COLOR, 1.0f, 0);
				...
				// Draw the other players
				{
					...
					for (int i = 0; i < MAX_USERS; ++i)
					{
						...
						if (players[i].bActive)
						{
							...
							D3DXMatrixScaling(&matScale, 0.0015f, 0.0015f, 0.0015f);
							D3DXMatrixTranslation(&matTransform, players[i].vRenderPos.x, players[i].vRenderPos.y, players[i].vRenderPos.z);
